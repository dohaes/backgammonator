...
public final class TestUtil {
	...
	/**
	 * Invokes method {@link DiceImpl#generateNext()} via reflection on <code>dice</code>
	 * @see {@link DiceImpl#generateNext()}
	 */
	public static void generateNext(DiceImpl dice) {
		if (generateNext == null) throw new RuntimeException("Method DiceImpl.generateNext not found!");
		try {
	...
	/**
	 * Invokes method {@link BackgammonBoardImpl#makeMove(PlayerMove move, Dice dice)} via reflection.
	 * The method is invoked on <code>board</code> with arguments <code>move</code> and <code>dice</code>.
	 * @see {@link BackgammonBoardImpl#makeMove(PlayerMove move, Dice dice)}
	 */
	public static boolean makeMove(BackgammonBoardImpl board, PlayerMove move, DiceImpl dice) throws Exception {
		if (makeMove == null) throw new RuntimeException("Method BackgammonBoardImpl.makeMove not found!");
		try {
			return ((Boolean)makeMove.invoke(board, move, dice)).booleanValue();
		} catch (Exception e) {
			throw new RuntimeException("Unexpected exception", e);
		}
	}
